...
var updateCmd = &cobra.Command{
	Use:   "update",
	Short: "Update grok client to a specific version",
	Long: `Update grok client to a specific version with interactive selection.
...
Examples:
  ...
  grok update --version v1.1.0

  # Update to match server version
  grok update --match-server

  # Include pre-releases in selection
  grok update --include-prerelease`,
	RunE: runUpdate,
}
...
var (
	...
	updateMatchServer       bool   // Match server version
)
...
func init() {
	...
	updateCmd.Flags().BoolVar(&updateMatchServer, "match-server", false,
...
func runUpdate(_ *cobra.Command, _ []string) error {
	...
	u, err := updater.NewUpdater("grok")
	if err != nil {
		return fmt.Errorf("failed to create updater: %w", err)
	}

	// Case 1: Match server version
	if updateMatchServer {
		return updateToMatchServer(u)
	}
	...
	// Case 3: Latest flag (backward compatibility)
	if updateLatest {
		return updateToLatest(u)
	}

	// Case 4: Interactive selection (new default)
	return updateInteractive(u)
}

func updateToMatchServer(u *updater.Updater) error {
	// Get server address from config
	cfg := GetConfig()
	serverAddr := cfg.Server.Addr

	logger.InfoEvent().Str("server", serverAddr).Msg("Querying server version")
	...
	serverVersion, err := checker.GetServerVersion(ctx)
	if err != nil {
	...
	if currentVersion == serverVersion {
		fmt.Printf("✓ Client already matches server version %s\n", serverVersion)
		return nil
	}

	// Update to server version
	fmt.Printf("Updating client from %s to %s (server version)...\n",
		currentVersion, serverVersion)

	return updateToVersion(u, serverVersion)
}
...
func updateToVersion(u *updater.Updater, targetVersion string) error {
	...
	// Validate assets
	if err := versionpkg.ValidateReleaseAssets(release, "grok"); err != nil {
...
func updateInteractive(u *updater.Updater) error {
	...
	if err := versionpkg.ValidateReleaseAssets(selectedRelease, "grok"); err != nil {
...
func performUpdate(u *updater.Updater, release versionpkg.Release) error {
	...
	fmt.Println("\nPlease restart the grok client to use the new version.")
	fmt.Println("You can verify the update by running: grok version")
